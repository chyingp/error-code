...
class AddErrorCode extends React.Component {
	...
	componentDidMount () {
		...
		if(this.props.id) {
			this.props.queryErrorCodeDetail({
				_id: this.props.id
			})
		}
	}
	...
	componentWillReceiveProps(nextProps) {
		...
		if(nextProps.getCodeStatus === 'success' && props.getCodeStatus !== 'success') {
			this.setState({
				...this.state,
				...nextProps.item
			})
				...
				<Panel header="编辑错误码">					
					<Form className="add-form">					
					    ...
					    <Button
					    	...
					    	onClick={this.onSubmit}>{btnText}</Button>					    
					</Form>	
				</Panel>			
			</div>					
		)
